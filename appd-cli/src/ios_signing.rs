...
fn resolve_macos(project: &Path, bundle_id: &str, device_id: &str) -> Result<Selection> {
    ...
    if candidates.is_empty() {
        return automatic_selection(project, bundle_id, device_id, &identities, &profiles).map_err(
            |error| {
                anyhow::anyhow!(
                    "no valid iOS development signing profile was found for bundle {bundle_id} and device {device_id}; automatic Xcode provisioning failed: {error}; set APPD_IOS_SIGNING_IDENTITY and APPD_IOS_PROVISIONING_PROFILE to override it"
                )
            },
        );
    }
...
fn automatic_selection(
    ...
    let embedded_profile = derived_data
        .join("Build/Products/Debug-iphoneos/AppdSigningProbe.app/embedded.mobileprovision");
...
fn automatic_team_id(
    ...
    if best.len() != 1 {
        bail!(
            "multiple Apple Development teams are available; set APPD_IOS_SIGNING_IDENTITY and APPD_IOS_PROVISIONING_PROFILE"
        );
    }
...
fn profile_relevance(profile: &Profile, bundle_id: &str) -> u8 {
    ...
    if pattern == bundle_id {
        3
    } else if pattern.starts_with("com.appd.") {
        2
    } else {
        1
    }
...
fn write_signing_probe(root: &Path, bundle_id: &str) -> Result<PathBuf> {
    let project = root.join("AppdSigningProbe.xcodeproj");
    ...
    fs::write(
        ...
        SIGNING_PROBE_PROJECT.replace("__APPD_BUNDLE_ID__", &pbx_escape(bundle_id)),
    )?;
    fs::write(root.join("main.m"), SIGNING_PROBE_SOURCE)?;
    fs::write(
        schemes.join("AppdSigningProbe.xcscheme"),
...
fn persist_profile(profile_id: &str, bytes: &[u8]) -> Result<PathBuf> {
    let directory = profile_cache_directory().context("resolve appd iOS signing cache")?;
    fs::create_dir_all(&directory).with_context(|| {
        format!(
            "create appd iOS signing profile cache: {}",
            directory.display()
        )
    })?;
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn automatic_signing_prefers_an_existing_appd_team() -> Result<(), Box<dyn std::error::Error>> {
        let identities = vec![
            Identity {
        ...
        let profiles = vec![
            Profile {
                ...
                application_identifier: "PERSONAL.com.appd.old-app".to_owned(),
    ...
    fn signing_probe_uses_the_requested_bundle_id() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            project
                .join("xcshareddata/xcschemes/AppdSigningProbe.xcscheme")
                .is_file()
        );
        assert_eq!(
