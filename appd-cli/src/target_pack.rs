...
/// A platform family supported by the appd CLI.
...
impl Target {
    ...
    pub const fn runtime_artifact_path(self) -> &'static str {
        match self.platform() {
            Platform::Android => "bin/libappd.so",
            Platform::Windows => "bin/appd-shell-windows.exe",
    ...
    pub const fn runtime_staging_path(self) -> &'static str {
        match self.platform() {
            Platform::Android => "runtime/libappd.so",
            Platform::Windows => "runtime/appd-shell-windows.exe",
            Platform::Ios | Platform::IosSimulator | Platform::Macos => {
                "runtime/frameworks/AppdRuntime.framework"
            }
        }
    }
...
/// Manifest included in each `appd` target pack.
///
...
impl TargetPackManifest {
    ...
    /// # Errors
    ///
    /// Returns an error when the CLI version does not equal the appd version
    /// recorded in the manifest.
    pub fn validate_cli_version(&self, cli_version: &str) -> Result<(), TargetPackError> {
        if self.appd_version == cli_version {
            Ok(())
        } else {
            Err(TargetPackError::IncompatibleAppdVersion {
                required: self.appd_version.clone(),
...
pub enum TargetPackError {
    ...
    MissingVersion,
    /// The target pack requires a different appd/CLI version.
    #[error("target pack was built for appd {required}, but this CLI is {actual}")]
    IncompatibleAppdVersion {
        /// appd version recorded in the target pack.
        required: String,
