...
final class AppdPluginBridge: NSObject, WKScriptMessageHandler {
  ...
  private let plugins: [String: any AppdPlugin]
  private var cancellations: [RequestKey: () -> Void] = [:]
  weak var webView: WKWebView?

  init(host: String, plugins: [any AppdPlugin]) {
    self.host = host
    var pluginsById: [String: any AppdPlugin] = [:]
  ...
  private func dispatch(
    ...
    let reply: AppdPluginReply = { [weak self] result in
  ...
  private func send(
    key: RequestKey,
    result: Result<Any?, AppdPluginError>,
    done: Bool
  ) {
    if done {
      cancellations.removeValue(forKey: key)?()
  ...
  private static func bootstrap(host: String) -> String {
    ...
    if (globalThis.location.origin === "https://\(host)") {
      globalThis.__appdNative = {
        onmessage: null,
        postMessage(message) {
          globalThis.webkit.messageHandlers.appd.postMessage(message);
        }
      };
    }
    """
