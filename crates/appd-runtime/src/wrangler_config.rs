...
/// Static asset subset of a Wrangler config that `appd` currently consumes.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl HtmlHandling {
    ...
    pub fn parse(value: &str) -> RuntimeResult<Self> {
        match value {
            ...
            _ => Err(RuntimeError::InvalidAssetConfig(format!(
...
pub fn resolve_config_path(
    ...
    Err(RuntimeError::WranglerConfigNotFound(
        reference_dir.to_path_buf(),
    ))
}
...
pub fn load_config(config_path: &Path) -> RuntimeResult<WranglerConfig> {
    ...
    let main = raw
        .main
        .ok_or_else(|| RuntimeError::MissingWranglerConfigField {
...
fn parse_config(config_path: &Path) -> RuntimeResult<RawWranglerConfig> {
    ...
    match extension {
        ...
        _ => Err(RuntimeError::UnsupportedWranglerConfigFormat(
            config_path.to_path_buf(),
        )),
    }
}

fn parse_jsonc_config(config_path: &Path, content: &str) -> RuntimeResult<RawWranglerConfig> {
    parse_to_serde_value(content, &ParseOptions::default()).map_err(|error| {
...
fn parse_toml_config(config_path: &Path, content: &str) -> RuntimeResult<RawWranglerConfig> {
    toml::from_str(content).map_err(|error| RuntimeError::InvalidWranglerConfig {
        path: config_path.to_path_buf(),
        message: error.to_string(),
    })
}
...
fn absolute_path(path: &Path) -> RuntimeResult<PathBuf> {
    if path.is_absolute() {
        Ok(path.to_path_buf())
    } else {
