...
use super::worker::prepare_android_bare_app;
...
pub(crate) fn build_android(
    ...
    let kotlin = app.join("src/main/kotlin");
    let java = app.join("src/main/java/com/appd/runtime");
    ...
    prepare_android_bare_app(&assets, pack_root, manifest, wrangler)?;
    install_wry_sources(pack_root, manifest, &kotlin)?;
    write_project(&staging, app_name)?;
...
fn install_runtime(pack_root: &Path, manifest: &TargetPackManifest, native: &Path) -> Result<()> {
    ...
    copy_file(runtime, native.join("libappd_runtime.so"))?;
    let bare = artifact_path(pack_root, manifest, &ArtifactKind::BareHostLibrary)?;
    copy_file(bare, native.join("libappd_bare.so"))
}

fn install_wry_sources(
    pack_root: &Path,
    manifest: &TargetPackManifest,
    kotlin: &Path,
) -> Result<()> {
    let source = artifact_path(pack_root, manifest, &ArtifactKind::AndroidKotlinDirectory)?;
    copy_dir_contents(&source, kotlin)
}

fn write_project(root: &Path, app_name: &str) -> Result<()> {
    fs::write(
    ...
    fs::write(
        root.join("app/src/main/java/com/appd/runtime/AppdActivity.java"),
        activity_java(),
    )?;
    Ok(())
}
...
fn manifest(app_name: &str) -> String {
    format!(
        "<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\">\n  <uses-permission android:name=\"android.permission.INTERNET\" />\n  <application android:label=\"{app_name}\" android:usesCleartextTraffic=\"false\">\n    <meta-data android:name=\"appd.host\" android:value=\"{app_name}.appd.local\" />\n    <activity android:name=\".AppdActivity\" android:exported=\"true\" android:launchMode=\"singleTask\" android:theme=\"@style/Theme.AppCompat.Light.NoActionBar\" android:configChanges=\"colorMode|density|fontScale|keyboard|keyboardHidden|layoutDirection|locale|mcc|mnc|navigation|orientation|screenLayout|screenSize|smallestScreenSize|touchscreen|uiMode\">\n      <intent-filter>\n        <action android:name=\"android.intent.action.MAIN\" />\n        <category android:name=\"android.intent.category.LAUNCHER\" />\n      </intent-filter>\n    </activity>\n  </application>\n</manifest>\n"
    )
}

fn activity_java() -> &'static str {
    "package com.appd.runtime;\n\npublic final class AppdActivity extends WryActivity implements Runnable {\n  @Override public native void run();\n}\n"
}

#[cfg(test)]
mod tests {
    #[test]
    fn android_project_requires_proxy_capabilities() {
        assert!(super::activity_java().contains("extends WryActivity"));
    }
