...
use appd_bundle::{AppLayout, app_host};
use appd_runtime::{Certificates, Config, Event, Runtime, frontend_url};
use base64::Engine;
...
impl ClientIdentity {
    fn install(certificates: &Certificates, host: &str) -> Result<Self> {
        let appd_runtime::Decision::PresentIdentity {
            certificate,
            private_key,
        } = certificates.decide(&appd_runtime::Challenge::ClientCertificate {
            host,
            previous_failures: 0,
        })
        else {
