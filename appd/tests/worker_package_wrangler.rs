...
fn load_config_error(path: &Path) -> TestResult<Error> {
...
fn rejects_missing_required_wrangler_fields() -> TestResult {
    ...
    assert!(matches!(
        error,
        Error::MissingConfigField { field: "main", .. }
    ));
...
fn rejects_invalid_wrangler_config_values_and_formats() -> TestResult {
    ...
    assert!(matches!(
        ...
        Error::UnsupportedConfigFormat(_)
    ));
