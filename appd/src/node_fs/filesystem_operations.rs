...
pub(super) fn path<'js>(ctx: &Ctx<'js>, value: Value<'js>) -> rquickjs::Result<String> {
    ...
    if path.chars().count() > crate::vfs::MAX_PATH_LENGTH {
    ...
    for part in path.split('/') {
        match part {
            ...
            part => {
                parts.push(part);
                if parts.len() > crate::vfs::MAX_PATH_SEGMENTS {
...
pub(super) fn stat_value<'js>(
    ...
    match result {
        ...
        Err(error)
            if error.kind() == crate::vfs::ErrorKind::NotFound && !options.throw_if_no_entry =>
        {
...
pub(super) fn vfs_call<T>(
    ctx: &Ctx<'_>,
    operation: impl FnOnce(&mut VirtualFileSystem) -> crate::vfs::Result<T>,
) -> rquickjs::Result<T> {
    let vfs = vfs_handle(ctx)?;
