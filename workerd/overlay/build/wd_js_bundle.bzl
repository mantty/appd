...
def _gen_compile_cache_impl(ctx):
    ...
    # The tool is built for the target platform (see the cfg = "target"
    # comment below), but a real device binary can't run on the build host.
    # Ship empty caches: V8 treats an invalid/absent compile cache as "compile
    # normally", so this only forgoes the startup-time optimization.
    if not ctx.attr._tool_is_executable[BuildSettingInfo].value:
        for out in outs:
...
_gen_compile_cache = rule(
    ...
    attrs = {
        ...
        "_tool_is_executable": attr.label(default = "//build/config:tool_is_executable"),
    },
)
