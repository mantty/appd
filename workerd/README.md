...
1. `scripts/fetch-upstream.py` downloads the pinned upstream release declared in
   ...
   checksum.
2. `scripts/apply-overlay.py` copies the small appd C ABI overlay into that
   source tree and applies ordered patches from `patches/`.
3. `scripts/build-sdk.py` builds `//src/workerd/server:appd-workerd-link-anchor`
   with Bazel, reads the link params for that target, and packages the static
   link inputs plus `appd_workerd.h` into an SDK directory under
