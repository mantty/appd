...
- Do not impose an appd request-count cap or return `503` because an appd
  queue is full. Let Tokio and the operating system apply resource
  backpressure, while appd accounts for memory and cancels work cleanly.
...
Every accepted request gets one Tokio blocking task. There is no appd request
...
- Concurrent requests remain independent. There is no appd request-count cap;
