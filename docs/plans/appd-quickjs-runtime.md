# appd QuickJS runtime

Status: draft

Supersedes: [request-scoped Bare realms and VFS](appd-request-realms-vfs.md)

Contract: [Workers JavaScript compatibility](appd-workers-node-compat.md)

## Objective

Replace Bare with an appd-owned Rust runtime embedding QuickJS-NG. Execute each
...
and current Node compatibility, the public appd runtime API, native shells,
...
- Use a published `rquickjs` release and its bundled QuickJS-NG sources. Carry
  no appd engine or binding patches.
- Implement ECMA-402 over pinned ICU4X crates and one shared immutable CLDR
  data image.
- Statically link QuickJS-NG and appd's native modules into each target.
...
- Run all QuickJS calls on one application JavaScript thread.
- Run the gateway and asynchronous host I/O on one Rust async-I/O thread.
...
- Keep at most 100 active request runtimes, also constrained by one app-wide
  memory budget. Queue at most 100 additional requests in FIFO order.
...
App users consume a prebuilt appd CLI and target packs. QuickJS-NG, `qjsc`,
...
Keep appd's explicit unsafe code inside the engine module. It implements the
...
external module is appd's statically linked `appd:host` module. Every other
...
At startup, appd loads an immutable `WorkerImage` containing the packaged
...
One dedicated JavaScript thread owns every QuickJS runtime and processes a
synchronous command queue. A separate Tokio current-thread executor owns the
gateway, timers, sockets, and non-blocking host I/O. The two threads exchange
bounded typed messages.

A native async operation records its execution and resource IDs. The
JavaScript thread retains the promise settlement functions and sends only
plain Rust data to the I/O thread. A completion returns to the JavaScript
thread, which looks up the execution, settles the promise, and drains only that
runtime's job queue. A completion for a disposed execution is dropped. No
other thread calls QuickJS.

Synchronous JavaScript runs to completion and may temporarily block other
JavaScript requests, while gateway and network progress continue. The
QuickJS interrupt handler reads cancellation and shutdown flags set by the I/O
or shell thread, allowing them to pre-empt long-running JavaScript.

`suspend()` stops new dispatch and pauses JavaScript jobs after the current
turn. Bounded network queues and timers remain owned by the I/O thread but do
not deliver JavaScript callbacks. `resume()` refreshes certificates before
dispatch and callbacks resume. Shutdown cancels all executions, closes the
gateway, and joins both threads.

### Native modules

Register one statically linked `appd:host` ES module assembled from focused
Rust modules:
...
`appd:host` is an internal API, not a security boundary, so every operation
...
Admission requires both an execution slot and memory budget. At most 100
requests are active. The gateway queues up to 100 additional requests in FIFO
order and returns HTTP 503 when that queue is full. Requests never share a
runtime, global object, native handle, or VFS.
...
`/tmp`, and appd-owned body and stream buffers use the same budget. Bounded
...
- perform server TLS with mandatory appd client authentication;
...
replace their `bare-*` dependency with an `appd:host` primitive or a standard
...
ICU4X primitives in `appd:host`. Store CLDR data once as immutable app-wide
...
2. Add the JavaScript and I/O threads and the CONNECT/TLS/HTTP gateway.
...
1. Add admission, `RequestExecution`, `RequestState`, resource ownership, and
   the unified teardown path.
...
- One hundred concurrent requests remain independent; excess requests queue
  without falling back to shared state.
...
- App-wide memory accounting covers QuickJS, VFS, and appd-owned buffers;
...
- Bare, BareKit, `bare-pack`, `appd-bare`, and runtime `bare-*` dependencies
