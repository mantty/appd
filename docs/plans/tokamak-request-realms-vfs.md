...
- Consume upstream Bare and `bare-realm`; do not patch, fork, or add private
  hooks to Bare.
- Keep the add-on tokamak-owned and written in Rust using Bare's supported
  add-on interfaces.
...
- Keep direct `bare-*` imports an explicit escape hatch. They remain outside
  the Workers compatibility contract and are not rejected by tokamak.
...
evaluates code in it, and destroys it. The tokamak add-on may compose that API
...
2. install the tokamak bootstrap in each Realm;
...
The bootstrap is the same tokamak runtime entrypoint currently used for a Worker,
...
Direct `bare-fs` access remains outside the Worker contract. The tokamak
`node:fs` implementation is the path that receives request-scoped VFS
semantics.

## Add-on boundary

The tokamak add-on should expose only host-side lifecycle operations to the
runtime shell:
...
4. Move the tokamak bootstrap and Worker dispatch into a Realm and implement the
   request/response/stream bridge.
5. Implement the Rust-backed Realm VFS and the tokamak `node:fs` adapter.
...
- The tokamak runtime can share immutable bundle storage; any inability to share
