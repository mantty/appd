# appd certificate hardening

Status: draft

## Objective

Keep appd's HTTPS origin and mutual TLS boundary without persisting private
...
The app origin remains `https://<appname>.appd.local/`. Certificate renewal
does not change the origin, website data, cookies, CORS behaviour, or service
worker scope.

## Required invariants

- Bare accepts only TLS connections for `<appname>.appd.local:443`.
...
the session CA for every appd connection.
...
- Do not write appd certificate material to `%LOCALAPPDATA%`.
...
threat model until appd uses an operating-system application boundary or a
...
- The common runtime writes the CA and client private keys to its certificate
  cache.
- Windows gives that cache a name-derived `%LOCALAPPDATA%` location.
...
- Windows leaves no certificate or key container after normal teardown and
  removes stale appd credentials after an unclean exit.
