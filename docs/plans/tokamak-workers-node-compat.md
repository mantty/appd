...
Run an ordinary Wrangler Worker with tokamak's current Workers Web API and
`nodejs_compat` surface on QuickJS-NG without exposing the host operating
system, the tokamak process, or unsupported Cloudflare services.

tokamak alpha has one compatibility contract. Every application receives the
current contract; `compatibility_date` and `compatibility_flags` are accepted
by the build for Wrangler compatibility but do not select historical tokamak
behaviour. A new tokamak release adopts a newer contract.
...
separate product decisions. WebAssembly is not part of the tokamak Worker
contract.

## Current QuickJS implementation

`tokamak/src/quickjs` creates a QuickJS runtime and context for each request and
...
modules under capability modules (`tokamak/src/fs`, `tokamak/src/streams`,
`tokamak/src/network`, `tokamak/src/events`, `tokamak/src/globals`, and
`tokamak/src/builtins`).
...
fixed tokamak request queue. This is an implementation baseline, not a claim
...
[tokamak QuickJS runtime plan](tokamak-quickjs-runtime.md). This plan owns
...
- `tokamak/src/quickjs` schedules each request-owned QuickJS runtime independently
...
- [ ] Accept Wrangler `compatibility_date` and `compatibility_flags` without
  changing the fixed tokamak contract. Add a pinned Wrangler/workerd fixture as
...
- [ ] Provide `HTMLRewriter` as a streaming Rust-backed transformation, or
  mark it explicitly unsupported in tokamak's contract until it exists.
...
- [ ] Give `process` deterministic Worker values for `argv`, `cwd`, `pid`,
  ...
  Workers/workerd oracle; they must never affect the tokamak process.
...
- [ ] Implement outbound `tls` clients over Rust TLS with supported options,
  ...
  tokamak's server certificates or private keys.
...
- [ ] Implement `EventSource` over the Rust fetch stream, including event
  ...
  `withCredentials` must match the reference contract; tokamak has no implicit
  outbound cookie jar unless that contract requires one.

### Worker services and tokamak-owned APIs
...
Follow the [tokamak QuickJS runtime plan](tokamak-quickjs-runtime.md) for engine,
...
- [ ] No supported API can reach host storage, host environment variables,
  ...
  another request.
- [ ] The existing Astro application runs unchanged through the normal tokamak
