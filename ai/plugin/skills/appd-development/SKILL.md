---
name: appd-development
description: "Build, run, debug, and design applications with appd. Use for appd projects, appd CLI commands, Worker-compatible application code, Wrangler configuration, native and web portability, WebView layout, Cloudflare binding limits, native plugins, mobile backgrounding, connection recovery, local state, or questions about how appd differs from Cloudflare Workers."
---

# appd development

Apply the following model whenever working on an appd application. appd is in early alpha; if the installed CLI or current repository conflicts with this skill, treat the installed version and current source as authoritative.

## Model appd correctly

- Treat appd as a cross-platform application framework, not as a Cloudflare hosting product and not as a frontend-only WebView wrapper.
...
- In a packaged native app, run the Worker-compatible server code locally on the user's device inside appd's embedded runtime. Package the Worker, static assets, native shell, and declared native plugins into the application.
- Load the UI from a stable secure `https://<app-host>.appd.local/` origin. Let the native shell route same-origin HTTP and WebSocket traffic to appd's local runtime.
...
- Treat compatibility as one-way: code written within appd's supported subset can also target Cloudflare Workers, but arbitrary Cloudflare Worker applications may depend on APIs or bindings appd does not provide.
- Use the project's normal framework or Cloudflare tooling for a web deployment. The current appd CLI has native targets only; there is no `appd build web` command.
...
- Expect server-side behavior during `appd dev` to come from the framework's host process. Do not use development mode as proof that a Worker API or Cloudflare binding exists in the packaged appd runtime.
...
```sh
appd devices
appd dev <device-selector> --project <project> -- <framework-dev-command>
appd build <platform[,platform...]> --project <project>
appd targets
```
...
- Let appd detect pnpm, Yarn, or npm from the project's lockfile when it runs the build.
...
- Check appd's current support before relying on a specific Cloudflare Worker or Node API. Similar syntax is not evidence that every Cloudflare or Node behavior exists.
...
- Treat Wrangler static assets as files packaged and routed by appd before Worker dispatch.
- Do not treat `env.ASSETS` as a full Cloudflare asset service; appd's static asset router is the supported path and `env.ASSETS.fetch()` is not currently implemented as an asset lookup.
- Do not dereference Cloudflare service bindings on an appd native path. appd does not currently provide bindings such as D1, KV, R2, Durable Objects, Queues, service bindings, Vectorize, Hyperdrive, Workers AI, Browser Rendering, Images, dispatch namespaces, mTLS, Pipelines, rate limiting, Secrets Store, Email Routing, or Analytics Engine.
...
- Use CSS pixels and the live viewport rather than physical display pixels or a fixed appd canvas size.
...
- On iOS foregrounding, expect the appd shell to restore its local gateway and update its internal proxy if necessary. Do not confuse gateway recovery with recovery of frontend JavaScript state or network sessions.
...
## Use native capabilities through appd plugins

- Import supported `@appd/*` frontend plugins for native capabilities instead of modeling those capabilities as Worker bindings.
...
- State an actual appd limitation when it affects the request, then offer choices that fit the application's requirements rather than silently replacing the architecture.
