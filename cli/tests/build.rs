...
use appd::worker_package::{AppLayout, decompress_worker_module, read_worker_manifest};
use assert_cmd::Command;
use predicates::str::contains;
use target_pack_format::{Target, TargetPackManifest, TargetPackVersion, write_manifest};
...
fn write_test_esbuild(root: &Path) -> TestResult {
    let path = root.join(target_pack_format::ESBUILD_EXECUTABLE);
...
fn create_target_pack(root: &Path, target: &str) -> TestResult<PathBuf> {
    ...
    fs::create_dir_all(root.join(target_pack_format::RUNTIME_JAVASCRIPT_DIRECTORY))?;
    fs::create_dir_all(
    ...
    write_manifest(
        root.join(target_pack_format::MANIFEST_FILE),
        &TargetPackManifest {
    ...
    Ok(root.join(target_pack_format::MANIFEST_FILE))
}
...
fn create_windows_inputs() -> TestResult<(tempfile::TempDir, PathBuf, PathBuf)> {
    ...
    fs::create_dir_all(pack.join(target_pack_format::RUNTIME_JAVASCRIPT_DIRECTORY))?;
...
cp "$input/runtime/appd-shell-windows.exe" "$output/$app_name.exe"
...
"#
    ...
    write_manifest(
        pack.join(target_pack_format::MANIFEST_FILE),
        &TargetPackManifest {
            schema_version: TargetPackVersion::CURRENT,
            appd_version: "0.1.0".to_owned(),
            target,
