...
use appd::worker_package::wrangler::WranglerConfig;
use target_pack_format::{ArtifactKind, Platform, TargetPackManifest};
use walkdir::WalkDir;
...
pub(crate) fn read_package_name(project: &Path) -> Result<String> {
    ...
    if !appd::worker_package::is_valid_app_name(name) {
...
pub(crate) fn run_entrypoint(
    ...
    input: &Path,
    output: &Path,
    target: target_pack_format::Target,
) -> Result<()> {
